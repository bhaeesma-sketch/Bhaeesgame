...
const Landing: React.FC<LandingProps> = ({ onPlay, onClaim, isClaimed, isConnected, walletAddress, onConnect }) => {
  ...
  const prizes = [
    { label: "1,000 BAZ JACKPOT", value: 1000, color: "#ffd700" }, // Index 0
    { label: "5 BAZ", value: 5, color: "#8b24ff" },              // Index 1
    { label: "0 BAZ", value: 0, color: "#150d22" },              // Index 2
    { label: "TRY AGAIN", value: 0, color: "#ff0099" },          // Index 3
    { label: "2 BAZ", value: 2, color: "#00f0ff" },              // Index 4
    { label: "1 BAZ", value: 1, color: "#150d22" }               // Index 5
  ];
  ...
  const spawnParticles = (count: number, isJackpot: boolean) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      // Jackpot particles are faster and more numerous
      ...
      // Longer duration for jackpot celebration
  ...
  const handleWeb3Spin = async () => {
    ...
    // Phase 1: Sign Transaction Simulation
    await new Promise(resolve => setTimeout(resolve, 1400));
    setTxStatus('vrf');
    
    // Phase 2: Awaiting Chainlink VRF Node
    await new Promise(resolve => setTimeout(resolve, 2200));
    setTxStatus('spinning');

    // 0.01% RTP LOGIC (1 in 10,000)
    ...
    let targetSegment = 0;
    if (!isJackpot) {
        // Uniformly pick one of the other 5 segments
        targetSegment = 1 + Math.floor(Math.random() * 5);
    }

    const rotations = 18; // High speed rotations
    const segmentAngle = 360 / prizes.length; // 60 degrees
    
    // Calculation: Align the center of the target segment with the top pointer (0 deg)
    // Wheel initial: segment 0 is at [ -30 to 30 ] if centered at top.
    // If our drawing starts at 0 (top), segment 0 is [0 to 60]. 
    // Midpoint of segment i is i*60 + 30.
    // We want (Midpoint + CurrentRotation) % 360 = 0
    // So target rotation offset is 360 - (i * 60 + 30)
    ...
    if (wheelRef.current) {
      const tl = gsap.timeline({
        onComplete: () => {
          ...
          if (isJackpot) {
            spawnParticles(400, true); // Massive burst for jackpot
          } else if (prizes[targetSegment].value > 0) {
            spawnParticles(80, false); // Standard burst for small wins
          }
      ...
      // Reset filter before starting
      gsap.set(wheelRef.current, { filter: 'blur(0px)' });

      // Cinematic Spin: Fast acceleration -> High speed blur -> Smooth Deceleration
  ...
  return (
    <div ref={containerRef} className="relative flex flex-col items-center justify-center min-h-[calc(100vh-120px)] overflow-hidden py-16 px-6 lg:px-20">
      {/* Background Ambience */}
      ...
      <div className="relative z-10 w-full max-w-[1700px] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-32 items-center">
        {/* Left: Branding & Action */}
        <div className="space-y-8 lg:space-y-12 text-center lg:text-left flex flex-col items-center lg:items-start">
          ...
          <p className="text-gray-400 text-lg md:text-xl xl:text-2xl max-w-xl mx-auto lg:mx-0 leading-relaxed font-sans font-light opacity-90 tracking-wide mt-10">
            Secure decentralized signup terminal. Verified 0.01% Grand Jackpot weighted via high-entropy VRF synthesis.
          </p>
          ...
          <div className="cursor-target glass-panel p-10 lg:p-16 rounded-[4rem] border-white/10 shadow-[0_0_100px_rgba(0,0,0,0.5)] relative overflow-hidden group border-t-primary/30 transition-all duration-700 hover:border-primary/60">
            ...
            {!isConnected && (
              <div className="absolute inset-0 z-50 bg-black/85 backdrop-blur-2xl flex flex-col items-center justify-center p-12 text-center animate-fade-in">
                ...
                <p className="text-gray-400 text-sm mb-12 leading-relaxed font-sans font-medium">Please link your wallet to authorize the Genesis VRF Reward Terminal and claim your starting credits.</p>
            ...
            {/* SVG SPIN WHEEL - Optimized 6-Segment UI */}
            <div className="relative w-72 h-72 lg:w-96 lg:h-96 mx-auto mb-16">
                ...
                {prizes.map((p, i) => {
                    ...
                    return (
                        <path
                            ...
                            fill={i === 0 ? 'url(#jackpotGrad)' : i % 2 === 0 ? 'url(#sliceGrad)' : '#1c142b'}
                            stroke="#000"
                            strokeWidth="0.4"
                        />
                    );
              ...
              >
                <div className="text-center">
                    <span className="block text-xs font-black text-white font-display uppercase tracking-widest">
                        {txStatus === 'signing' ? 'SIGN' : txStatus === 'vrf' ? 'VRF' : spinning ? 'ROLL' : isClaimed ? 'SYNC' : 'SPIN'}
                    </span>
                    {!spinning && !isClaimed && isConnected && (
                        <span className="block text-[8px] text-primary font-bold mt-2 tracking-[0.3em]">INITIATE</span>
                    )}
                </div>
              </div>
            </div>
            ...
            <div className="space-y-6">
                {txStatus === 'finalized' ? (
                  <div className="bg-primary/10 border border-primary/40 p-8 rounded-[2.5rem] text-center animate-fade-in relative overflow-hidden">
                    <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-transparent via-gold to-transparent animate-shimmer"></div>
                    ...
                    <p className="text-gray-400 text-sm mb-8 leading-relaxed font-sans font-medium">
                      {winAmount > 0 
                        ? `Genesis protocol successfully credited ${winAmount.toLocaleString()} BAZ to your identity. Access the modules to spend.`
                        : `Terminal synchronized. Access the main lobby to initialize your gaming session.`}
                    </p>
                    <button onClick={onPlay} className="w-full py-5 rounded-2xl bg-gradient-to-r from-gold to-yellow-600 text-black font-display font-black uppercase tracking-[0.2em] text-xs hover:bg-white transition-all shadow-neon-gold active:scale-95">
                      START PLAYING
                    </button>
                  </div>
                ) : spinning ? (
                    <div className="bg-black/90 border border-primary/40 p-6 rounded-3xl space-y-4 animate-pulse">
                        <div className="flex items-center gap-4">
                            <span className="material-symbols-outlined text-primary text-3xl animate-spin">cyclone</span>
                            <div className="flex-1">
                                <p className="text-[10px] text-white font-black uppercase tracking-widest">
                                    {txStatus === 'signing' ? 'Awaiting Protocol Handshake...' : txStatus === 'vrf' ? 'Chainlink Oracle Synthesis...' : 'Finalizing Result...'}
                                </p>
                                <p className="text-[8px] text-gray-500 font-mono mt-1 uppercase">ORACLE_LINK: 0x{Math.random().toString(16).slice(2, 28)}</p>
                            </div>
                        </div>
                  ...
                  <button onClick={onPlay} className="w-full py-6 lg:py-7 rounded-3xl bg-black/40 border border-white/10 text-white font-display font-black uppercase tracking-[0.4em] text-xs lg:text-sm hover:bg-white hover:text-black transition-all shadow-2xl active:scale-[0.98]">
                    RE-ENTER LOBBY
                  </button>
                ) : (
